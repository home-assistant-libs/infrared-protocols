...
files under `infrared_protocols/codes/`. `get_codes` returns a
...
codes = get_codes("lg/tv")  # does not open/read the .ir file
...
codes = get_codes("my_device", base_dir=Path("~/ir-codes").expanduser())
```
